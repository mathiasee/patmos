...
{
	char msg[100];
	//snprintf(msg,100, "Foer move_window\n");
	//send_uart(msg);
	fs->wflag = 0; fs->winsect = 0xFFFFFFFF;		/* Invaidate window */
	if (move_window(fs, sect) != FR_OK) return 4;	/* Load boot record */
	
	//snprintf(msg,100, "vol number %d winsect: %lu\n", fs->drv,fs->winsect);
	//send_uart(msg);
	//for(int i=0; i<512;i++){
	//snprintf(msg,100, "AA55 check: %d\n", (ld_word(&fs->win[BS_55AA]) != 0xAA55));
	//send_uart(msg);
	//}
	...
	//snprintf(msg,100, "ld_dword(&fs->win[BS_FilSysType] %06lX\n", ld_dword(&fs->win[BS_FilSysType]));
	//send_uart(msg);
	//snprintf(msg,100, "ld_dword(&fs->win[BS_FilSysType32] %06lX\n", ld_dword(&fs->win[BS_FilSysType32]));
	//send_uart(msg);
	if ((ld_dword(&fs->win[BS_FilSysType]) & 0xFFFFFF) == 0x544146) return 0;	/* Check "FAT" string */
	if ((ld_dword(&fs->win[BS_FilSysType32]) & 0xFFFFFF) == 0x544146) return 0;	/* Check "FAT" string */
	//if ((ld_dword(&fs->win[BS_FilSysType32]) & 0xFFFFFF) == 0xD44146) return 0;	/* Check "FAT" string */
#if _FS_EXFAT
...
*/
	if (fmt == 2 || (fmt < 2 && LD2PT(vol))) {	/* Not an FAT boot sector or forced partition number */
		for (i = 0; i < 4; i++) {			/* Get partition offset */
			...
			/*snprintf(msg,100, "ld_dword(&pt[8]) : %lu\n", ld_dword(&pt[8]));
			send_uart(msg);*/
		}
